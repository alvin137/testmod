...
public class MTRMContainer extends Container {
	...
	public MTRMContainer(InventoryPlayer p_i1808_1_) {
		...
		for (int y = 0; y < 3; ++y)
			for (int x = 0; x < 3; ++x)
				this.addSlotToContainer(new Slot(this.craftMatrix, x + y * 3, 22 + x * 26, 21 + y * 26));
		for (int y = 0; y < 3; ++y)
			for (int x = 0; x < 9; ++x)
				this.addSlotToContainer(new Slot(p_i1808_1_, x + y * 9 + 9, 8 + x * 18, 99 + y * 18));
		for (int x = 0; x < 9; ++x)
			this.addSlotToContainer(new Slot(p_i1808_1_, x, 8 + x * 18, 157));
		...
		if ((i >= 0 && i <= 9) || i == RETURN_SLOT_ID) // Fake slots
		{
			if (mousebtn == 2) {
				...
				if (stackHeld != null) {
					...
					if (!(i == 0 || i == RETURN_SLOT_ID))
						newStack.getItem().setMaxStackSize(1);
				...
				if (stackHeld != null) {
					...
					if (stackSlot != null && stackHeld.isItemEqual(stackSlot) && (i == 0 || i == RETURN_SLOT_ID)) {
						...
						if (stackSlot.getMaxStackSize() + 1 > max)
							stackSlot.getItem().setMaxStackSize(max);
						getSlot(i).putStack(stackSlot);
					} else {
						stackHeld.getItem().setMaxStackSize(1);
						getSlot(i).putStack(stackHeld);
					}
				} else {
					if (stackSlot != null) {
						stackSlot.getItem().setMaxStackSize(stackSlot.getMaxStackSize() - 1);
						if (stackSlot.getMaxStackSize() == 0)
							getSlot(i).putStack(null);
					}
				}
			}
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int slots) {
		if (slots < 10 || slots == RETURN_SLOT_ID)
			((Slot) inventorySlots.get(slots)).putStack(null);
		return null;
	}

	// public boolean func_94530_a(ItemStack p_94530_1_, Slot p_94530_2_)
	// {
	// return p_94530_2_.inventory != craftResult && p_94530_2_.inventory !=
	// returnSlot && super.func_94530_a(p_94530_1_, p_94530_2_);
	// }
}