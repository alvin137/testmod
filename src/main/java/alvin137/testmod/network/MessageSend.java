...
public class MessageSend implements IMessage {
	...
	private String getMarker(boolean noIngredients) {
		...
		if (!noIngredients)
			marker.append(shapeless ? " SHAPELESS" : " SHAPED");
	...
	private String getScript(boolean noIngredients, ArrayList<ArrayList<String>> ingredients) {
		...
		if (!noIngredients)
			script.append(shapeless ? "Shapeless" : "Shaped");
	...
	private MessageResponse makeScript() {
		...
		if (data[0] == null)
			return new MessageResponse(MessageResponse.Status.NO_OUT);
		if (!remove && noIngredients)
			return new MessageResponse(MessageResponse.Status.NO_IN);
		...
		try {
			...
			if (!i.hasNext())
				return new MessageResponse(MessageResponse.Status.WRITE_ERROR,
						"Cannot find " + marker + " . Did you edit the file manually?");
